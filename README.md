...
患者情報・疾患・バイタルサイン・看護記録・薬剤情報を、ブラウザだけで登録・閲覧・編集・削除できます。
英語を主表示、日本語を補助表示にした医療システム風のバイリンガルUIを採用しています。
...
- 全主要画面の共通検索から患者ID・氏名の部分一致で患者詳細へ移動（候補表示、キーボード操作、Escape・外側クリック対応）
...
### Patient Detail
- Patient Overview / Medical Information / Diagnoses / Medications / Vital Signs / Nursing Notes の各セクション
- 患者ID・氏名・年齢・性別・病室・主な疾患をまとめた概要ヘッダー
- 登録されたアレルギーを赤系の警告カードで強調（「なし」「未登録」などの明記時は中立表示）
...
- 個別削除（確認ダイアログあり）
- 2件以上ある体温・収縮期血圧・拡張期血圧・脈拍・SpO₂の日時・数値推移を簡易グラフ表示（正常・異常の評価なし）

### Medications
- Regular Medications / 定期薬と PRN Medications / 臨時薬を患者ごとに分けて管理
- 薬剤名・用量・単位・服用タイミング／使用条件・前回使用日時・メモを登録
- 服用タイミング／使用条件はテンプレート選択と自由入力の両方に対応
- 個別編集・確認ダイアログ付き個別削除・二重保存防止
- 登録・表示・履歴管理だけを行い、投与可否・用量・投与間隔などの医療判断は行いません
...
- すべてのデータ（患者情報・疾患・バイタル・看護記録・薬剤情報）は、`medichart-lite:app-data:v1` というキーで **ブラウザの Local Storage** に JSON 形式で保存されます。
- 外部サーバーへの送信は一切行いません。
...
interface AppData {
  ...
  medications: Medication[];  // patientId で Patient.id を参照
  sampleDataLoaded: boolean;   // サンプルデータの重複投入を防ぐフラグ
}
```

旧データに `medications` がない場合は空配列を補完するため、既存の患者・バイタル・看護記録を保持したまま利用できます。
